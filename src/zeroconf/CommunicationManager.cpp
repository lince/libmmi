...
namespace zeroconf{
	...
	void CommunicationManager::sendToDevice(vector<string>* args) {

	}

	string CommunicationManager::getDeviceId() {

	}

	void CommunicationManager::release() {
	...
	void CommunicationManager::run() {
		...
		if (!(simplePoll = avahi_simple_poll_new())) {
			ERROR(logger, "Failed to create simple poll object.\n");
		} else {
			name = avahi_strdup("Multimodal Event");
			...
			if (!client) {
				ERROR(logger, ((string) "Failed to create client: ") +
...
//				avahi_simple_poll_loop(simplePoll);
	...
	int CommunicationManager::startSocket() {
		...
		if ((sock = socket(AF_INET, SOCK_STREAM, 0)) == -1) {
			ERROR(logger, "Não foi possível criar um socket");
			return 1;
		}
		...
		if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &trueVar, sizeof(int))
				== -1) {
			ERROR(logger, "Não foi possível setar opção de permitir"
		...
		if (bind(sock, (struct sockaddr *)&server_addr, sizeof(struct sockaddr))
				== -1) {
			ERROR(logger, "Unable to bind");
			return 3;
		}

		if (listen(sock, 5) == -1) {
			ERROR(logger, "Erro na chamada listen");
			return 4;
		}
		...
		while (true) {
			ERROR(logger, "Waiting for client on port " +
					Functions::numberToString(port) );

			if (! (connected = accept(sock, (struct sockaddr*)&client_addr,
					(socklen_t*)&sin_size))){
				ERROR(logger, "Unable to connect");
				return 5;
			}

			// TODO O número da porta que está sendo mostrado é diferente toda vez
			// que um evento é recebido. Pensei que seria sempre 1234. Isso está
			// certo?
			ERROR(logger, ((string) "I got a connection from (") +
					inet_ntoa(client_addr.sin_addr) + ", " +
					Functions::numberToString(ntohs(client_addr.sin_port)) + ")");

			bytes_received = recv(connected, &bytesToBeReceived, sizeof(int), 0);

			DEBUG(logger, "bytes_received = " + bytes_received);
			DEBUG(logger, "bytesToBeReceived = " + bytesToBeReceived);
			//bytesToBeReceived = 10000;
			...
			printf("Primeiro caracter: %d\n", recv_data[0]);

			MMIManager* eim = MMIManager::getInstance();
			XMLData* xmlData = new XMLData();
			xmlData->payload = recv_data;
			xmlData->length = bytes_received;
